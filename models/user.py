...
class User(object):
    def __init__(self, email, username,password, first_name, last_name, gender, phone, picture, _id=None):
        self.email = email
        ...
        self._id = uuid.uuid1().int>>64 if _id is None else _id
    ...
    @classmethod
    def register(cls, email, username, password, first_name, last_name, gender, phone, picture):
        ...
        if user is None:
            ...
            new_user = cls(email, username, hashpass, first_name, last_name, gender, phone, picture)
    ...
    def get_chrips(self):
        return Topic.find_by_author_id(self._id)

    def get_lists(self):
        return

    def new_chrip(self, title, description):
        chrip = Topic(author=self.username,
                    title=title,
    ...
    @staticmethod
    def show_all_list(username):
        return [user for user in Database.find(collection='users', query={'username': username})]
